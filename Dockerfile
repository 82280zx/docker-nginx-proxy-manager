...
RUN \
    ...
    sed-patch 's|80;|8085;|' /etc/nginx/conf.d/default.conf && \
    sed-patch 's|"80";|"8085";|' /etc/nginx/conf.d/default.conf && \
    sed-patch 's|listen 80;|listen 8085;|' /opt/nginx-proxy-manager/templates/letsencrypt-request.conf && \
    sed-patch 's|:80;|:8085;|' /opt/nginx-proxy-manager/templates/letsencrypt-request.conf && \
    sed-patch 's|listen 80;|listen 8085;|' /opt/nginx-proxy-manager/templates/_listen.conf && \
    sed-patch 's|:80;|:8085;|' /opt/nginx-proxy-manager/templates/_listen.conf && \
    sed-patch 's|listen 80 |listen 8085 |' /opt/nginx-proxy-manager/templates/default.conf && \
