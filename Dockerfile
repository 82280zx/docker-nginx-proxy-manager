...
RUN \
    ...
    sed-patch 's|81 default|81 default|' /etc/nginx/conf.d/production.conf && \
    ...
    sed-patch 's|80;|80;|' /etc/nginx/conf.d/default.conf && \
    sed-patch 's|"80";|"80";|' /etc/nginx/conf.d/default.conf && \
    sed-patch 's|listen 80;|listen 80;|' /opt/nginx-proxy-manager/templates/letsencrypt-request.conf && \
    sed-patch 's|:80;|:80;|' /opt/nginx-proxy-manager/templates/letsencrypt-request.conf && \
    sed-patch 's|listen 80;|listen 80;|' /opt/nginx-proxy-manager/templates/_listen.conf && \
    sed-patch 's|:80;|:80;|' /opt/nginx-proxy-manager/templates/_listen.conf && \
    sed-patch 's|listen 80 |listen 80 |' /opt/nginx-proxy-manager/templates/default.conf && \

    # Change the HTTPs port 443 to the unprivileged port 4443.
    sed-patch 's|443 |443 |' /etc/nginx/conf.d/default.conf && \
    sed-patch 's|"443";|"443";|' /etc/nginx/conf.d/default.conf && \
    sed-patch 's|listen 443 |listen 443 |' /opt/nginx-proxy-manager/templates/_listen.conf && \
    sed-patch 's|:443;|:443;|' /opt/nginx-proxy-manager/templates/_listen.conf && \
